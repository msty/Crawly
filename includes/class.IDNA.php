...
class Net_IDNA
{
    ...
    */
    ...
    /**
     * Encodes domain to xn-- representation
     *
     * @param string $url
     *
     * @return string
     */
    public static function encodeDomain($url)
    {
        $info = parse_url($url);
        if (empty($info['host']) || empty($info['scheme'])) {
            return $url;
        }
        if (preg_match('#[а-яА-Я]#u', $info['host'])) {
            $instance = self::getInstance();
            $url = str_replace($info['scheme'] . '://' . $info['host'], $info['scheme'] . '://' . $instance->encode($info['host']), $url);
//            $url = $info['scheme'] . '://' . $instance->encode($info['host']) . isset($info['path']) ? $info['path'] : '';
        }
        return $url;
    }

    /**
     * Decodes domain from xn-- representation
     *
     * @param string $url
     *
     * @return string
     */
    public static function decodeDomain($url)
    {
        $originalUrl = $url;
        $url = str_ireplace('://www.', '://', $url);
        $info = parse_url($url);
        if (!$info || empty($info['host']) || empty($info['scheme']) || mb_strpos($info['host'], 'xn--') !== 0) {
            return $originalUrl;
        }
        $instance = self::getInstance();
        $url = str_replace($info['scheme'] . '://' . $info['host'], $info['scheme'] . '://' . $instance->decode($info['host']), $url);
        return $url;
    }
}
