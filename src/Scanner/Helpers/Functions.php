...
class Functions
{
    ...
    public static function trim_url_length($url, $maxLength = 80)
    {
        if (mb_strlen($url) > intval($maxLength)) {
            $length = Functions::mb_strrpos_array($url, array('/', '-', '&'), 80 - mb_strlen($url)) + 1;
            $url = mb_substr($url, 0, $length) . '...';
        }
        return $url;
    }

    public static function mb_strrpos_array($haystack, $needle, $offset = 0)
    {
        $return = false;
        if(!is_array($needle))
            $needle = array($needle);
